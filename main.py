...
def load_vgg(sess, vgg_path):
    ...
    return w1, keep_prob, layer_3_out, layer_4_out, layer_7_out
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    vgg_layer4_out_transformed = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, strides=(1, 1), padding='same',
                                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    output = tf.add(output, vgg_layer4_out_transformed)

    output = tf.layers.conv2d_transpose(output, num_classes, 4, 2, padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    vgg_layer_3_out_transformed = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, strides=(1, 1), padding='same',
                                                   kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    output = tf.add(output, vgg_layer_3_out_transformed)
    ...
    with sess.as_default():
        ...
        for i in range(epochs):
            ...
            for image, label in get_batches_fn(batch_size):
                batch_no += 1
                print('training,  EPOCH: {}, BATCH_NO: {}'.format(i, batch_no))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # OPTIONAL: Augment Imag
        # es for better results
        ...
        logits, training_operation, cost = optimize(final_layer, correct_label, learning_rate, num_classes)

        train_nn(sess, EPOCHS, BATCH_SIZE, get_batches_fn, training_operation, cost, input_image,
             correct_label, keep_prob, learning_rate)
